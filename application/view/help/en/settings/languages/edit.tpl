<p>To prepare your store for usage in another languages you have to translate appropriate words and sentences
from English to the necessary language.</p>

<div class="tasks">
<fieldset>
<legend>Things you can do</legend>
	<ul>	
		<li><a href="{self}#user_interface">Translate user interface</a></li>
		<li><a href="{self}#product_info">Translate product info</a></li>
		<li><a href="{self}#find_missing_translations">Find missing translations</a></li>
		<li><a href="{self}#translate_directly">Translate (a word or sentence) directly from user interface</a>

		</li>
	</ul>
</fieldset>
</div>


<h3 id="user_interface">Translate user interface</h3>

<!--
<p>There are two ways to translate the interface - either by using the <strong>word tree</strong> or 
<a href="{self}#user_interface">Live Translation</a> tool.</p>
-->

<p>To translate interface:</p>
<ol>
	<li>Select a group on the word tree:</li>
	<img src="image/doc/language/edit/tree.bmp"/>
	<li>To supply translations enter the corresponding translations in the fields provided.</li>
	<p>(Take a look at the following example:)</p>
	<img src="image/doc/language/edit/cursor.bmp"/>
	<ul>
		<li>here <span style="background: #ffffcc">text field</span> is a field for your translations;</li>
		<li><span style="background: #ffffcc">_user_registration</span> is a variable name;</li>
		<li>and <span style="background: #ffffcc">New Account Registration</span> is the default translation which is used if the text field is left empty.</li>
	</ul>
	<p class="note"><strong>Note</strong>: You can also enter multiple lines of text in the text area - just click the "Down Arrow" key on your keyboard to switch to multi-line input field.</p>
	<li>Translation filter - you can filter results by entering a certain criteria in the search field. The search will be carried out only within the word group chosen in the first step. You can search all files (?) by marking the "Search all files" checkbox.</li>
	<p>You can have keywords(?) filtered by their current status in terms of translation process. The following 
filtering options are available:</p>

<img src="image/doc/language/edit/translation_filter.bmp"/>
<ul>
	<li><span style="background: #ffffcc">All</span> – show all the existing words</li>
	<li><span style="background: #ffffcc">Translated</span> – display only translated items</li>
	<li><span style="background: #ffffcc">Not translated</span> – show items that have not been 
	translated</li>
</ul>
</ol>

<h3 id="product_info">Translate product info</h3>	
	
<p>You can translate products, categories, attributes and other data directly from their management pages, 
for example, products modification page. The fields for entering translations are typically placed below the
 main data form.</p>
 
<!-- 
<h3 id="find_specific_word">Find and translate a specific word or sentence</h3>

<p>If you need to find a specific word, use a search field in the Translation Filter area. The search is carried
 out on their default values in English and translations as well.</p>	
<img src="image/doc/language/edit/search.bmp"/>

<p>The results that match your search criteria appear below automatically.</p>
-->

<h3 id="find_missing_translations">Find missing translations</h3>	
<p><ins>necessary section?</ins></p>
<p>To find missing translations, select "Not translated" in the Translation Filter:</p>
<ins>tune up, test</ins>
<img src="image/doc/language/edit/translation_filter2.bmp"/>
<ul>
	<li><span style="background: #ffffcc">Not translated</span> – show items that have not been translated</li>
</ul>

<h3 id="translate_directly">Translate a word or sentence directly from user interface</h3>	
	
<p>Live Translation is a LiveCart tool which allows you to translate a word or sentence directly from the
 page you see it (in). To use Live Translation, you have to turn it on the Customization Mode (...)</p>
<p><strong>Live Translation</strong> is a real-time translation tool which enables you to translate any 
interface text (element) directly in the page you see it in. To do that you have to turn on Live Translation
 in the ... mode. Any interface phrase can be translated simply by clicking <strong>Live Translation</strong>
  once you hover the mouse cursor over an element and a pop-up menu appears. We highly recommend to use
   <strong>Live Translation</strong> when you are not sure about the keyword's exact place in the interface.</p>


	
{helpSeeAlso}
	{see settings.languages}
{/helpSeeAlso}	