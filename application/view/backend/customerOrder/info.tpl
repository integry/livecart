{form handle=$form action="controller=backend.order action=update" id="orderInfo_`$order.ID`_form" onsubmit="Backend.CustomerOrder.Editor.prototype.getInstance(`$order.ID`, false).submitForm(); return false;" method="post"}
    ...
    <fieldset class="error">
        <div class="formDiv">
            {$order.ShippingAddress.countryID} {$order.ShippingAddress.stateName}, {$order.ShippingAddress.city}, {$order.ShippingAddress.address1} {if $order.ShippingAddress.postalCode}({$order.ShippingAddress.postalCode}){/if}
        </div>
	</fieldset>
    ...
    {if $order.ShippingAddress.sddress2}
        ...
        <fieldset class="error">
            <div class="formDiv">
                {$order.ShippingAddress.countryID} {$order.ShippingAddress.stateName}, {$order.ShippingAddress.city}, {$order.ShippingAddress.address2} {if $order.ShippingAddress.postalCode}({$order.ShippingAddress.postalCode}){/if}
    	    </div
        </fieldset>
    {/if}    
    ...
    <fieldset class="error">
        <div class="formDiv">
            {$order.BillingAddress.countryID} {$order.BillingAddress.stateName}, {$order.BillingAddress.city}, {$order.BillingAddress.address1} {if $order.BillingAddress.postalCode}({$order.BillingAddress.postalCode}){/if}
	    </div>
    </fieldset>
...
<hr />
    ...
    <label for="order_{$order.ID}_isFinalized">{t _is_finalized}</label>    
    <fieldset class="error">
        <div class="formDiv">
            {if $order.isFinalized}{t _yes}{else}{t _no}{/if}
	    </div>
	</fieldset>
