<fieldset class="inventory {if 'INVENTORY_TRACKING_DOWNLOADABLE'|config}downloadableInventory{/if}">
	...
	<p>
		...
		<label class="checkbox" for="product_{$cat}_{$product.ID}_isUnlimitedStock">{tip _unlimited_stock}</label>
	</p>

	<div class="stockCount">
		<p {if $form|isRequired:"stockCount"}class="required"{/if}>
			<label for="product_stockCount_{$cat}_{$product.ID}">{tip _items_in_stock}:</label>
