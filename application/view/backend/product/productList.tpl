{foreach from=$productList key=key item=product}
	<tr class="{if ($offset + $key) is even}even{else}odd{/if}">
		...
		<td class="cell_name"><a href="{link controller=backend.product action=edit id=$product.ID}" onclick="window.openProduct({$product.ID}); return false;">{$product.name_lang}</a></td>
