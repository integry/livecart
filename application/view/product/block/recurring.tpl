{% if config('DISPLAY_PRICES') && isRecurring %}
	{foreach from=recurringProductPeriods item=period name=rpp}
		<tr class="productRecurring" id="productRecurring_[[period.ID]]">
			<td class="withRadio">
				<input type="radio" name="recurringID" {% if smarty.foreach.rpp.first %}checked="checked"{% endif %}
			...
			<td>
				...
				<label for="recurringPeriod_[[period.ID]]" class="period">
					...
					{t _every}
					<span class="price">{% if period.periodLength == 1 %}{t `periodTypesSingle[period.periodType]`}{% else %}[[period.periodLength]] {t `periodTypesPlural[period.periodType]`}{% endif %}</span>
					{math equation="a * b" a=period.periodLength|default:0 b=period.rebillCount|default:0 assign="x"}
					{% if x > 0 %}{t _for} <span class="price">[[x]]</span> {t `periodTypesPlural[period.periodType]`}, [[period.rebillCount]] {t _rebill_times}{% endif %}
				</label>
