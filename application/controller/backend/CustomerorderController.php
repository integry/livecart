...
class CustomerOrderController extends ActiveGridController
{
	...
	{
		$orderGroups = array(
			array('id' => self::TYPE_ALL, 'title' => $this->translate('_all_orders'), 'children' => 
				array(array('id' => self::TYPE_CURRENT, 'title' => $this->translate('_current_orders'), 'children' => 
					array(
						array('id' => self::TYPE_NEW, 'title' => $this->translate('_new_orders')),
						array('id' => self::TYPE_PROCESSING, 'title' => $this->translate('_processing_orders')),
						array('id' => self::TYPE_AWAITING, 'title' => $this->translate('_awaiting_shipment_orders')))))));

		/*
		if (CustomerOrder::hasRecurringorderBy())
		{
			$orderGroups = array_merge($orderGroups, array(
				array('id' => self::TYPE_RECURRING, 'rootID' => self::TYPE_ALL, 'title' => $this->translate('_recurring')),
				array('id' => self::TYPE_RECURRING_ALL, 'rootID' => self::TYPE_RECURRING, 'title' => $this->translate('_recurring_all')),
				array('id' => self::TYPE_RECURRING_EXPIRED, 'rootID' => self::TYPE_RECURRING, 'title' => $this->translate('_recurring_expired')),
				array('id' => self::TYPE_RECURRING_CANCELLED, 'rootID' => self::TYPE_RECURRING, 'title' => $this->translate('_recurring_canceled')),
			));
		}
		*/

		$orderGroups[0]['children'] = array_merge($orderGroups[0]['children'], array(
				array('id' => self::TYPE_SHIPPED, 'title' => $this->translate('_shipped_orders')),
				array('id' => self::TYPE_RETURNED, 'title' => $this->translate('_returned_orders')),
				array('id' => self::TYPE_CANCELLED, 'title' => $this->translate('_cancelled_orders'))));

		//array('id' => self::TYPE_CARTS, 'title' => $this->translate('_shopping_carts'), 'rootID' => self::TYPE_ROOT),

		$this->set('orderGroups', array('children' => $orderGroups));
	}
	...
	protected function getDefaultColumns()
	{
		return array(
			...
			'order\CustomerOrder.invoiceNumber',
			//'User.fullName',
			//'User.email',
			'order\CustomerOrder.dateCompleted',
			'order\CustomerOrder.totalAmount',
...
//		$this->set('hideShipped', $shipableShipmentsCount > 0 ? $hideShipped : 1);
	...
	public function exportDetailedAction()
	{
		...
		// fetch detailed data
		$f = new ARSelectFilter(new INCond('OrderedItem.customerOrderID', $ids));
		$data = array();
	...
	protected function setDefaultSortOrder($filter)
	{
		$filter->orderBy('order\CustomerOrder.dateCompleted DESC, order\CustomerOrder.ID DESC');
	}
				...
				$cond = new AndChainCondition(array(
					...
					'CustomerOrder.isRecurring = :CustomerOrder.isRecurring:', array('CustomerOrder.isRecurring' => 1),
						new OrChainCondition(array(
							new MoreThanCond('CustomerOrder.rebillsLeft', 0),
							new IsNullCond('CustomerOrder.isRecurring')
						))
					)
				);
