...
class CsCartImport extends LiveCartImportDriver
{
...
*/
	...
	{
		return array('categories', 'currencies', 'orders', 'order_data', 'languages', 'manufacturers', 'manufacturer_descriptions', 'product_descriptions');
	}
	...
	public function getNextLanguage()
	{
		...
		if (!$data = $this->loadRecord('SELECT * FROM ' . $this->getTablePrefix() . 'languages'))
		{
			return null;
		}

		$this->languages[$data['lang_code']] = $data['lang_code'];

		$lang = ActiveRecordModel::getNewInstance('Language');
		$lang->setID($data['lang_code']);
		$lang->isEnabled->set('Y' == $data['avail']);
