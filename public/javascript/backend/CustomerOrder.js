...
Backend.Shipping = {
    activeListCallbacks: 
    {
        beforeDelete: function(li){ 
            if(confirm(Backend.Shipping.messages.areYouSureYouWantToDelete)) 
            {
                return Backend.Shipping.links.deleteRelated + "/?ShippingID=" + this.getRecordId(li);
            }
        },
        ...
        beforeSort: function(li, order){ 
            return Backend.Shipping.links.sort + "?target=" + this.ul.id + "&" + order
        },
        afterSort: function(li, response){ }
    },
    addProductToList: function(productID, ShippingID)
    {
        var self = this;
        new Ajax.Request(this.links.related + "/?ShippingID=" + ShippingID , {
           method: 'get',
           onSuccess: function(response) {
                ...
                else
                {
                    try
                    {
                        ...
                        relatedList.addRecord(ShippingID, response.responseText, true);
...
Backend.Shipping.Group = {};
Backend.Shipping.Group.Callbacks =
{
    beforeDelete: function(li) { 
        if(confirm(Backend.Shipping.Group.Messages.areYouSureYouWantToDelete)) 
        {
            return Backend.Shipping.Group.Links.remove + "/" + this.getRecordId(li);
        }
    },
    afterDelete: function(li, response) {
        if(!response.error) {
    ...
    beforeSort: function(li, order) { 
        return Backend.Shipping.Group.Links.sort + '&' + order;
    },
    afterSort: function(li, response) { 
    },
    ...
    {
        if(!Backend.Shipping.Group.Controller.prototype.getInstance(li.down('.productRelationshipGroup_form')))
        {
            return Backend.Shipping.Group.Links.edit + "/" + this.getRecordId(li);
        }
        else
        {
            with(Backend.Shipping.Group.Controller.prototype.getInstance(li.down('.productRelationshipGroup_form')))
            {
