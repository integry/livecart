...
Backend.ShippingClass = Class.create();
Backend.ShippingClass.prototype =
{
	Messages: {},
	...
	Callbacks: {
		...
		{
			if(confirm(Backend.ShippingClass.prototype.Messages.confirmRemove))
			{
				return Backend.ShippingClass.prototype.Links.remove + "/" + this.getRecordId(li);
			}
		},
		...
		beforeEdit:	 function(li)
		{
			...
			{
				return Backend.ShippingClass.prototype.Links.edit + '/' + this.getRecordId(li)
			}
			else
			{
				...
				{
					Backend.ShippingClass.prototype.getInstance(newClassForm).hideNewForm();
				}
		...
		{
			return Backend.ShippingClass.prototype.Links.sort + '?target=class_classesList&' + order
		},
	...
	{
		if(!Backend.ShippingClass.prototype.Instances[$(root).id])
		{
			Backend.ShippingClass.prototype.Instances[$(root).id] = new Backend.ShippingClass(root);
		}

		return Backend.ShippingClass.prototype.Instances[$(root).id];
	},
	...
	save: function()
	{
		...
		var action = this.nodes.form.elements.namedItem('ID').value
			? Backend.ShippingClass.prototype.Links.update
			: Backend.ShippingClass.prototype.Links.create;
